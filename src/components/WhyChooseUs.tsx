...
const features: { icon: React.ElementType; titleKey: TranslationKey; descKey: TranslationKey }[] = [
  { icon: IndianRupee, titleKey: "why_card1_title", descKey: "why_card1_desc" },
  { icon: UserCheck,   titleKey: "why_card2_title", descKey: "why_card2_desc" },
  { icon: Clock,       titleKey: "why_card3_title", descKey: "why_card3_desc" },
  { icon: MapPin,      titleKey: "why_card4_title", descKey: "why_card4_desc" },
];
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {features.map((f, i) => (
              <motion.div key={i} variants={fadeUp} transition={{ duration: 0.5 }} className="text-center p-6 rounded-xl bg-primary-foreground/5 backdrop-blur-sm border border-primary-foreground/10">
                ...
                <h3 className="font-serif font-bold text-primary-foreground mb-2">{t(f.titleKey)}</h3>
                <p className="text-primary-foreground/70 text-sm">{t(f.descKey)}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
